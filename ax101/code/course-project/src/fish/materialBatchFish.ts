...
import { ProductionOrderFishes } from './productionOrderFish'
...
type State = UnknownState | DefinedState
...
const emitInputMaterialConsumed = (
    ...
    pond.emit(
        materialBatchTag
            .withId(batchId)
            .and(ProductionOrderFishes.tags.productionOrderTag.withId(orderId)),
        {
            eventType: 'inputMaterialConsumed',
            batchId,
            orderId,
...
export const MaterialBatchFishes = {
    ...
    of: (batchId: string): Fish<State, MaterialBatchEvent | InputMaterialBatchScannedEvent> => ({
            ...
            switch (event.eventType) {
                ...
                case 'inputMaterialConsumed': {
                    if (state.state === 'defined') {
                        ...
                        const lastConsumedByMachine = state.consumedByMachine[event.orderId] || 0
                        state.consumedByMachine[event.orderId] = lastConsumedByMachine + 1

                        const lastConsumedByOrder = state.consumedByMachine[event.orderId] || 0
    ...
    all: (): Fish<Record<string, boolean>, InputMaterialBatchScannedEvent> => ({
